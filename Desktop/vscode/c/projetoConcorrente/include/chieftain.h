...
#define __CHIEFTAIN_H__
    ...
    *============================================================================*/
    ...
    typedef struct chieftain
    {
        ...
        pthread_mutex_t escolhaMesaMutex;        // protege cadeiras e pratos
        pthread_cond_t  liberouLugarCond;     // acorda quem está esperando
        int *cadeira_ocupada;        // [table_size]  0/1
        int *cadeira_tipo;           // berserker de quem está sentado
        int *prato1, *prato2;// quais 2 pratos a cadeira pegou (pra liberar depois)
        int *prato_ocupado;          // [table_size]  0/1

        pthread_mutex_t comeramMutex;        // protege barreira
        pthread_cond_t  comeramCond;
        int comeramCont;

        pthread_mutex_t escolhaGodMutex;
        int rezasNormais[NUMBER_OF_GODS];
