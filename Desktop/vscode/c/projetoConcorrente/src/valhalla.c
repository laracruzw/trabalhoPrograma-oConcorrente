...
void valhalla_init(valhalla_t *self)
{
    ...
    pthread_mutex_init(&self->contaPrayersLock, NULL);
...
{
    /* TODO: Adicionar código aqui se necessário! */
    pthread_mutex_destroy(&self->contaPrayersLock);
...
void valhalla_pray(valhalla_t *self, god_t god)
{
    ...
    pthread_mutex_lock(&self->contaPrayersLock); // adicionei o lock pois prayers daria race condition
    self->prayers[god]++; 
    pthread_mutex_unlock(&self->contaPrayersLock);
